...
router.post('/:collectionId/books/:bookId', async (req, res) => {
    try {
        ...
        // Check if the user has access to this collection
        if (!collection.users.some(user => user._id.toString() === req.user._id.toString())) {
            console.log(`User ${req.user._id} does not have access to collection ${collectionId}`);
            return res.status(403).json({error: 'You do not have access to this collection'});
        }
        ...
        if (book) {
            console.log(`Found existing book: ${book.title} (${book.id}) with MongoDB _id: ${book._id}`);
        } else {
            console.log(`Book with id ${bookId} not found in database, creating new book`);
        }
        ...
        if (!book) {
            try {
                ...
                await book.save();
                console.log(`Created new book in database: ${book.title} (${book.id})`);
            } catch (error) {
                console.error('Error creating book:', error);
                return res.status(500).json({error: `Failed to fetch or create book: ${error.message}`});
            }
        }
        ...
        console.log(`Checking if book ${book.title} (${book.id}) with MongoDB _id ${book._id} is already in collection`);
        console.log(`Collection books: ${JSON.stringify(collection.books.map(id => id.toString()))}`);
        
        const bookAlreadyExists = collection.books.some(existingBookId => {
            const match = existingBookId.toString() === book._id.toString();
            if (match) {
                console.log(`Book already exists in collection: ${existingBookId} === ${book._id}`);
            }
            return match;
        });
        
        if (bookAlreadyExists) {
            console.log(`Book ${book.title} (${book.id}) already exists in collection, returning 400`);
        ...
        const updatedCollection = await Collection.findById(collectionId).populate('books');
