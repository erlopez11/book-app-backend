...
router.get("/:bookId/bookLog", async (req, res) => {
  try {
    const currentUser = await User.findById(req.user._id);
    const bookLogItem = currentUser.bookLog.find(
      (logEntry) => logEntry.book === req.params.bookId
    );
    res.status(200).json(bookLogItem ?? null);
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: error.message });
  }
